...
impl PyUserModel {
    ...
    /// Merges the rectangle anchored at `(row, column)` spanning `width`
    /// columns and `height` rows, keeping only the anchor value and clearing
    /// the covered cells' content. Undoable as a single history entry.
    pub fn merge_cells(
        &mut self,
        sheet: u32,
        row: i32,
    ...
    pub fn get_merge_cells<'py>(&self, py: Python<'py>, sheet: u32) -> PyResult<Bound<'py, PyAny>> {
        let regions = self.model.get_merge_cells(sheet).map_err(to_py_err)?;
        to_python(py, &regions)
    }

    /// Returns the merged region covering `(row, column)` (anchor or covered),
    /// or `None` if the cell is not merged
    pub fn get_merge_cell<'py>(
        &self,
        py: Python<'py>,
        sheet: u32,
        row: i32,
